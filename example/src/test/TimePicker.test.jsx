...
describe('<TimePicker />', () => {
    ...
    it('Calls props.setValue with value + intervalStep minutes when clicking increase-time-button', () => {
        ...
        mountComponent({
            state: [new Date(2015, 11, 15, 12, 15), setValue]
        });

        wrapper.find('label[name="increment"] > button').simulate('click');

        expect(setValue.mock.calls.length).toBe(1);
        expect(setValue.mock.calls[0][0]).toEqual(new Date(2015, 11, 15, 12, 30));
    });
    ...
    it('Calls props.setValue with value - intervalStep minutes when clicking decrease-time-button', () => {
        ...
        expect(setValue.mock.calls.length).toBe(1);
        expect(setValue.mock.calls[0][0]).toEqual(new Date(2015, 11, 15, 12));
    });

    it('Cycles within the same day when props.restrictToDay is true', () => {
        let setValue = jest.fn();
        mountComponent({
            state: [new Date(2015, 11, 15, 23, 45), setValue],
        ...
        expect(setValue.mock.calls.length).toBe(1);
        expect(setValue.mock.calls[0][0]).toEqual(new Date(2015, 11, 15, 0));
        
        setValue = jest.fn();
        mountComponent({
            state: [new Date(2015, 11, 15), setValue],
            restrictTimeToDay: true
        });
        ...
        expect(setValue.mock.calls.length).toBe(1);
        expect(setValue.mock.calls[0][0]).toEqual(new Date(2015, 11, 15, 23, 45));
        ...
        expect(setValue.mock.calls.length).toBe(1);
        expect(setValue.mock.calls[0][0]).toEqual(new Date(2015, 11, 16));
        ...
        expect(setValue.mock.calls.length).toBe(1);
        expect(setValue.mock.calls[0][0]).toEqual(new Date(2015, 11, 14, 23, 45));
    });
});